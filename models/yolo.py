...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.training:
            # (bs, x, 85)
            # print(len(z), len(z[0]), len(z[0][0]))
            origin_output = torch.cat(z, 1)
            output_bboxes_nums = 100
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
                ...
                BottleneckCSP, C3, C2, C2f, C1, C3TR, C3SPP, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x}:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C3, C2, C2f, C1, C3TR, C3Ghost, C3x}:
        ...
        elif m in {vanillanet_5, vanillanet_6, vanillanet_7}:
            m = m(*args)
            c2 = m.channel
        else:
            c2 = ch[f]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='yolov5_vani.yaml', help='model.yaml')
    ...
    # Options
    if 1 or opt.line_profile:  # profile layer by layer
        model = model.fuse()
